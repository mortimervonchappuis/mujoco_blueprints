"""
Cameras provide a rendered image for each time step. A :class:`Camera` can be attached to a :class:`mujoco_blueprints.body.Body` 
or to the :class:`mujoco_blueprints.world.World` directly. If a Camera is attached to a :class:`mujoco_blueprints.body.Body` 
in a :class:`Agent <mujoco_blueprints.agent.Abent>` its rendered images are contained in the Agents observations. All 
cameras can be viewed when :meth:`mujoco_blueprints.world.World.view` by setting the ``camera`` in the ``Rendering`` 
section.

Camera Orientation
------------------

Similarly to :class:`mujoco_blueprints.light.Light` Cameras can either be fixed in orientation locally/globally or 
...
Recording
---------
To record videos with a Camera you can use the methods :meth:`World.start_recording <mujoco_blueprints.world.World.start_recording>`, 
:meth:`World.step <mujoco_blueprints.world.World.step>` and :meth:`World.stop_recording <mujoco_blueprints.world.World.stop_recording>`.
For more details refer to :meth:`start_recording <mujoco_blueprints.world.World.start_recording>`.
...
from mujoco_blueprints.utils.geometry import RADIANS_TO_DEGREES, DEGREES_TO_RADIANS, TAU
import time
...
class Camera(blue.CameraType, blue.thing.CyclicalThing, blue.thing.MoveableThing):
	...
	"""
	This class is available through the shortcut :class:`mujoco_blueprints.Camera <Camera>`.
	...
	def start_recording(self, 
		...
		used if the camera is already attached within a :class:`World <mujoco_blueprints.world.World>`.

		For details read :meth:`World.start_recording <mujoco_blueprints.world.World.start_recording>`.
	...
	def looking(self, 
		...
		pos : list [ int | float] | np.ndarray | blue.MoveableThingType | None
			If set, it can either be a (global) position or a :class:`MoveableThing <mujoco_blueprints.thing.moveable.MoveableThing>` 
	...
	def resolution(self) -> np.ndarray:
		"""
		The resolution is used for :attr:`observation` rendering and :meth:`recording <mujoco_blueprints.world.World.start_recording>`. 
		Rendering in :meth:`World.view <mujoco_blueprints.world.World.view>` uses OpenGL defaults.
