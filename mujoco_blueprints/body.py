...
* :class:`Geom <mujoco_blueprints.geoms.BaseGeom>`: This Thing is used to fill in the Body with matter. Geoms have a shape, mass, friction etc.

* :class:`Site <mujoco_blueprints.sites.BaseSite>`: Sites similar to Geoms have shape, but do not effect the simulation physics. Instead :class:`Sensor <mujoco_blueprints.sensors.BaseSensor>` instances as well as :class:`Actuator <mujoco_blueprints.actuators.BaseActuator>` can be attached to it.

* :class:`Camera <mujoco_blueprints.camera.Camera>`: If attached to the Body, the Camera can be view trough :meth:`World.view <mujoco_blueprints.world.World.view>` by setting the :attr:`rendering <mujoco_blueprints.camera.Camera.rendering>` camera value to the Cameras name. If the Body is included in an :class:`Agent <mujoco_blueprints.agent.Agent>` (not existing right now) the camera will be available in the Agents observations.

* :class:`Placeholder <mujoco_blueprints.placeholder.Placeholder>`: A placeholder can be used to specify a position and orientation relative to the Body to which it is attached, that is of special interest. If a Thing is attached to a placeholder it is instead attached to its parent in a position and orientation, that matches attachment to the placeholder.

* :class:`Actuator <mujoco_blueprints.actuators.BaseActuator>`: Actuators are used to apply force to the kinematic tree at the node they are attached to. If the Body is included in an :class:`Agent <mujoco_blueprints.agent.Agent>` (not yet implemented) the actuators input will appear in the Agents action space.

* :class:`Light <mujoco_blueprints.light.Light>`: Light sources illuminate the space around them, but are not necessary for the model to be visible, since a base luminescence is presence always present.

* :class:`Joint <mujoco_blueprints.joints.BaseJoint>`: This Thing is used to move the :class:`Body`:. If a force is applied to the Body it moves along the degrees of freedom defined by the Joint.
...
class Body(blue.BodyType, blue.thing.MoveableThing, blue.thing.NodeThing):
	"""
	This class is available through the shortcut :class:`mujoco_blueprints.Body <Body>`.

	Attributes from base classes :class:`MoveableThing <mujoco_blueprints.thing.moveable.MoveableThing>` and :class:`NodeThing <mujoco_blueprints.thing.node.NodeThing>` are inherited.
	
	Attributes
	----------
	bodies, cameras, geoms, sites, joints, lights, cameras, actuators, placeholders : :class:`View <mujoco_blueprints.utils.view.View>`
		All children that have been attached to the Body are retrieved by the attribute as a :class:`View <mujoco_blueprints.utils.view.View>`.
	"""
	@blue.restrict
	def __init__(self, 
	...
	def targeting_cameras(self):
		"""
		Targeting Cameras contain :class:`Camera <mujoco_blueprints.camera.Camera>` that track this Body.
		
		Returns
		-------
		:class:`View <mujoco_blueprints.utils.view.View>`
		"""
	...
	def targeting_lights(self):
		"""
		Targeting Lights contain :class:`Light <mujoco_blueprints.light.Light>` that track this Body.
