...
the standard orientation attributes of :class:`mujoco_blueprints.thing.moveable.MoveableThing`. Mujoco uses the 
:attr:`Light.dir` attribute internally, so blueprint synchronizes all changes in ``euler``, ``alpha``, ``beta``, 
``gamma`` with ``dir`` internally.

Similarly to :class:`mujoco_blueprints.camera.Camera` Lights can either be fixed in orientation locally/globally or 
...
from mujoco_blueprints.utils.geometry import TAU, PI
...
class Light(blue.LightType, blue.thing.CyclicalThing, blue.MoveableThing):
	...
	"""
	This class is available through the shortcut :class:`mujoco_blueprints.Light <Light>`.
	...
	def mode(self) -> str:
		"""
		This argument works identically to :attr:`mujoco_blueprints.camera.Camera.mode`. Possible values are:
