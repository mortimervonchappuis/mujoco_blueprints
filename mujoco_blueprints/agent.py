"""
To make mujoco_blueprints amneable for multi agent settings the :class:`Agent` class provides easily structured
access to the relevant Things for observation (:class:`Sensors <mujoco_blueprints.sensors.BaseSensor>` and :class:`Cameras <mujoco_blueprints.Camera>`) 
and action (:class:`Actuators <mujoco_blueprints.actuators.General>`).

Agents are used similar to :class:`Bodies <mujoco_blueprints.bodies.Body>` with some minor differences.
