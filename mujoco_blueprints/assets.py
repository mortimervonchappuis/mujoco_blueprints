...
.. note::
	...
	handeled through a front facing ThingType. Assets are :mod:`UniqueThings <mujoco_blueprints.thing.unique>`.
"""
...
class MaterialAsset(blue.MaterialAssetType, BaseAsset, blue.thing.ColoredThing):
	"""
	See :mod:`Material <mujoco_blueprints.material>`
	...
	def texuniform(self) -> bool:
		"""
		For :class:`BoxTextures <mujoco_blueprints.texture.Box>`, this attribute controls how 
		...
		The value “true” means that the 2D :class:`PlaneTexture <mujoco_blueprints.texture.Plane>` 
...
class TextureAsset(blue.TextureAssetType, BaseAsset):
	"""
	See :mod:`Texture <mujoco_blueprints.texture>`
...
class MeshAsset(blue.MeshAssetType, BaseAsset, blue.thing.MoveableThing):
	...
	"""
	MeshAssets handle access to the data attributes used to define the :class:`Mesh <mujoco_blueprints.geoms.Mesh>`. 
	All vertex, face, normals and texture coordinate data are stored in the MeshAssets :class:`MeshCache <mujoco_blueprints.cache.MeshCache>` 
	...
	while their plural form :attr:`vertecies`, :attr:`faces`, :attr:`cache.texcoords <mujoco_blueprints.cache.MeshCache.texcoords>` and :attr:`cache.normals <mujoco_blueprints.cache.MeshCache.normals>` are used 
	...
	built : bool
		...
		:class:`Meshes <mujoco_blueprints.geoms.Mesh>` might use the same :class:`MeshAsset` in which case 
		it should be built only once.
	cache : blue.CacheType
		The :class:`MeshCache <mujoco_blueprints.cache.MeshCache>` which stores the Meshs data.
	filename : str
	...
	def __init__(self, 
		...
		cache : blue.ThingType | None, optional
			The :class:`Cache <mujoco_blueprints.cache.MeshCache>` which stores the Mesh data.
	...
	def size(self, size: np.ndarray|list[int|float]) -> None:
		...
		size : np.ndarray | list[int | float]
			Changing this parameter scales the vertecies directly in the :class:`Cache <mujoco_blueprints.cache.MeshCache>` 
...
class HFieldAsset(blue.HFieldAssetType, BaseAsset, blue.thing.MoveableThing):
	"""
	See :class:`HField <mujoco_blueprints.geoms.HField>`
