...
:class:`Tendons <mujoco_blueprints.tendon.Tendon>` can be used to string together multiple Things. The can provide additional 
...
treated specially with the :meth:`Path.bind <mujoco_blueprints.tendon.Path.bind>` method replacing :meth:`NodeThing.attach <mujoco_blueprints.thing.node.NodeThing.attach>` within 
...
* :attr:`name <mujoco_blueprints.tendon.Tendon.name>`
* :attr:`limited <mujoco_blueprints.tendon.Tendon.limited>`
* :attr:`range <mujoco_blueprints.tendon.Tendon.range>`
* :attr:`frictionloss <mujoco_blueprints.tendon.Tendon.frictionloss>`
* :attr:`margin <mujoco_blueprints.tendon.Tendon.margin>`
* :attr:`springlength <mujoco_blueprints.tendon.Tendon.springlength>`
* :attr:`stiffness <mujoco_blueprints.tendon.Tendon.stiffness>`
* :attr:`damping <mujoco_blueprints.tendon.Tendon.damping>`
...
.. note::
	Keep in mind that Fixed Joint Tendons do not support :attr:`width <mujoco_blueprints.tendon.Tendon.width>` and are hence invisible.
...
---------------------------
Spatial Tendons are bind to :class:`Sites <mujoco_blueprints.sites.BaseSite>` and :class:`Geoms <mujoco_blueprints.geoms.BaseGeom>` and support path splitting. The following rules
must be satisfied for mujoco to accept the build.

* Every path must start and end with a :class:`Site <mujoco_blueprints.sites.BaseSite>`.
* Every :class:`Geom <mujoco_blueprints.geoms.BaseGeom>` must be sandwiched between two :class:`Sites <mujoco_blueprints.sites.BaseSite>`.
...
When binding a :class:`Geom <mujoco_blueprints.geoms.BaseGeom>` to a Tendon, an optional ``side_site`` argument may be 
...
Using the method :meth:`split <mujoco_blueprints.tendon.Path.split>` a pulley is introduced that splits the path into `n` 
...
Copying Tendons takes special attention since Tendons introduce cyclical references (i.e. multiple children of a :class:`NodeThing <mujoco_blueprints.node.NodeThing>`
...
class Tendon(blue.TendonType, blue.NodeThing, blue.ColoredThing, blue.FocalThing):
	...
	def max_act_force(self) -> float:
		...
		-------
		float
			Maximum force applicable by an :class:`Actuator <mujoco_blueprints.actuator.BaseActuator>`
		"""
	...
	def min_act_force(self) -> float:
		...
		-------
		float
			Minimum force applicable by an :class:`Actuator <mujoco_blueprints.actuator.BaseActuator>`
		"""
	...
	def damping(self) -> float:
		...
		If possible, :attr:`Joint.damping <mujoco_blueprints.joints.BaseJoint.damping>` should be used 
		for stability.
...
class Path(blue.PathType):
	...
	A path can be strung along multiple :class:`Sites <mujoco_blueprints.sites.BaseSite>`, 
	:class:`Joints <mujoco_blueprints.joints.BaseJoint>` and :class:`Geoms <mujoco_blueprints.geoms.BaseGeom>`. 
	...
	def split(self, number: int) -> tuple:
		"""
		If a pulley should split the Tendon into :math:`n` :class:`Paths <mujoco_blueprints.tendon.Path>`, this method 
